...
def play(hh,mm,ss):
    ...
    pygame.mixer.init()
    pygame.mixer.music.load("alarm_beep.mp3")
...
submit = Button(gui,text = "Set Alarm",command=lambda: (thread_run(hourTime.get(),minTime.get(),secTime.get()),clearTxt(),start())).place(x =220,y=260)
def Close():
    gui.destroy()
def deletealarm():
    delete_btn=Button(gui,text="Delete")
    # if(alarm_run==i):
    delete_btn.config(command=stop1())
    delete_btn.place(x=10,y=340)
